...
| `OPENAI_EMBEDDING_MODEL` | `text-embedding-3-large` | Modelo de vectores para indexar y consultar. | Valor por defecto del repo: `text-embedding-3-large` (mejor calidad relativa, más coste y vectores más largos). Alternativa habitual: `text-embedding-3-small` (1536 dimensiones nativas, más barato y rápido). Tras **cualquier** cambio de modelo o de `OPENAI_EMBEDDING_DIMENSIONS`, la colección Chroma existente deja de ser compatible: `POST /ingest/reset` y vuelve a ingerir. |
| `OPENAI_EMBEDDING_DIMENSIONS` | *(vacío/sin definir)* | Tamaño de vector de salida (**MRL / Matryoshka**) en modelos `text-embedding-3-*`. | **Vacío** = dimensión **nativa** del modelo: **1536** con `text-embedding-3-small`, **3072** con `text-embedding-3-large`. Con MRL puedes fijar un tamaño menor (p. ej. **1024** o **256** con `large`, **512** con `small`) para ahorrar espacio; debe coincidir con el índice ya guardado. Ver [OpenAI: embeddings](https://platform.openai.com/docs/guides/embeddings). **Importante:** Chroma fija la dimensión al crear la colección; no se mezclan vectores de distinto tamaño en el mismo `CHROMA_COLLECTION_NAME`. |
...
- Si cambias **`CHUNK_SIZE`** o **`CHUNK_OVERLAP`**, los trozos (y a menudo los embeddings) dejan de alinearse con un reindexado limpio: usa **`POST /ingest/reset`** y vuelve a ingerir.
- Si cambias **`OPENAI_EMBEDDING_MODEL`** o **`OPENAI_EMBEDDING_DIMENSIONS`**, Chroma exigirá la **misma** dimensión de vector que al crear la colección. Un índice creado con p. ej. `small` (1536) **no** admite inserciones con `large` a tamaño completo (3072): la ingesta falla con un error del tipo *expecting embedding with dimension of 1536, got 3072*. Solución: **`POST /ingest/reset`** y subir de nuevo los documentos (no basta ajustar el `.env` sin vaciar el índice; si además cambias de modelo, debes re-embeddar todo para coherencia semántica).
